...
def run_single_deter_dynamics(dir_path):
    ...
    c_win_params = [1/15, 0.05, FACTOR1, 0, NUM_OF_GENERATIONS, GROWTH_RATE, INITIAL_NUM_OF_BIRDS, CARRYING_CAPACITY]
    l_win_params = [1/15, 0.05, FACTOR2, 0, NUM_OF_GENERATIONS, GROWTH_RATE, INITIAL_NUM_OF_BIRDS, CARRYING_CAPACITY]
    co_ex_params = [1/15, 0.05, FACTOR3, 0, NUM_OF_GENERATIONS, GROWTH_RATE, INITIAL_NUM_OF_BIRDS, CARRYING_CAPACITY]
    ...
    subplot_titles = (f'Colony death factor: {FACTOR1}', f'Colony death factor: {FACTOR2}', f'Colony death factor: {FACTOR3}')
...
def run_deter_heatmap(dir_path):
    ...
    for i, rate in enumerate(PANDEMIC_RATES):
        for j, factor in enumerate(DEATH_FACTORS):
            ...
            params = [rate, 0.05, factor, 0, NUM_OF_GENERATIONS, GROWTH_RATE, INITIAL_NUM_OF_BIRDS, CARRYING_CAPACITY]
    ...
    Plotter.plot_heatmap(mat, DEATH_FACTORS, PANDEMIC_RATES, xaxis_title="Death factor", yaxis_title="Pandemic rate", legend_title="Fraction of colony birds")
    mk_heatmap_header(new_path, DETER_MODEL, "colony_birds")
    save_heatmap_data(new_path, DEATH_FACTORS, PANDEMIC_RATES, mat)

def single_run_example(dir_path):
    ...
    fig = Plotter.plot_birds_numbers_scatter_plot(colony_birds, lone_birds, f"pandemic rate: {RATE1}")
    fig.show()
